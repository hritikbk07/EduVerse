      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="flex items-center gap-2 opacity-0 -translate-y-2 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-out">
            <a
              href={onEdit || onDelete ? `/instructor/courses/${course._id}/lessons` : `/courses/${course._id}`}
              className="px-3 py-1.5 text-xs font-bold uppercase tracking-wider bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 hover:text-indigo-300 border border-indigo-500/20 rounded-xl transition-all shadow-lg active:scale-95"
              title={onEdit || onDelete ? "Manage Lessons" : "View Details"}
            >
              {onEdit || onDelete ? "Lessons" : "View Details"}
            </a>
            {onEdit && (
              <button
        ...
        <div className="flex-grow">
          ...
          {/* 👇 INSTRUCTOR (ADDED) */}
          <p className="text-indigo-400 text-sm font-semibold mb-6">
